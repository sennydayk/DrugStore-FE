  ...
  const deletequestionhandler = async (questionid: number) => {
    ...
    try {
      const response = await axios.delete(
        `https://drugstoreproject.shop/product/question?question-id=${encodedquestionId}`,
        {
          headers: {
            Token: token,
  ...
  return (
    <>
      <div className="qnaitem_qna">
        <div className="qnaitem_qnastatus">{qna.question_status}</div>
        <div className="qnaitem_question" onClick={qestionclickhandler}>
          {qna.question.length > 10
            ? qna.question.substring(0, 10) + "..."
            : qna.question}
        </div>
        <div className="qnaitem_username">{qna.user_name}</div>
        <div className="qnaitem_createdat">{qna.created_at}</div>
        <button
          className="qnaitem_updatebutton"
          onClick={() => updatequestionhandler(qna.question_id)}
        >
          수정
        </button>
        <button
          className="qnaitem_deletebutton"
          onClick={() => deletequestionhandler(qna.question_id)}
        >
          삭제
        </button>
      </div>
